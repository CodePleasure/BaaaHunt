...
func _ready():
	...
	var arrows = ["Arrow_Lightning"]
	$"Armature/Skeleton/Bip001 R HandBoneAttachment/Bow".load_arrows(arrows)
...
func process_model_rotation(delta):
	...
	var rotation_rad = Helper.xy2rad(input_movement_vector_normalized) + $Camera.rotation.y
